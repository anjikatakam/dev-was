...
<?php } ?>
					<form data-toggle="validator" method="post" action="">
						<div class="form-group margin-bottom-20">
							...
							<select class="form-control" name="id" required>
                                ...
                                foreach($get_regions as $regions){
                                    if($regions->id == $country_details[0]->regionid){
                                        echo "<option value='".$regions->id."' selected='selected'>".$regions->region."</option>";
                                    }else{
                        ...
                        <div class="form-group">
							...
							<input type="text" class="form-control" id="inputName" value="<?php echo $country_details[0]->country?>" placeholder="Enter Country" name="country" required>
						</div>
						
                        <div class="form-group">
							<button type="submit" class="btn btn-primary waves-effect waves-light">Update Country</button>
						</div>
					</form>
				</div>
				<!-- /.box-content -->
